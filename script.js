...
function playerPointsText() {
   ...
   ctx.fillText(playerScoreText.text, canvas.width * 0.75, canvas.height * 0.1);
}
...
function createNewAsteroid(x, y, radius, size) {
   ...
   let asteroid = {
      ...
      vertices: Math.floor(
         Math.random() * (ASTEROIDS_VERTEX_AVG + 1) + ASTEROIDS_VERTEX_AVG / 2
      ),
   ...
   if (oldRadius > 0) {
      switch (true) {
         case oldRadius == Math.ceil(ASTEROIDS_SIZE_PX / 2):
            ...
            currentAsteroidsArray.push(
               createNewAsteroid(asteroidX, asteroidY, oldRadius / 2, 'medium')
            );
            currentAsteroidsArray.push(
               createNewAsteroid(asteroidX, asteroidY, oldRadius / 2, 'medium')
            );
         ...
         case oldRadius == Math.ceil(ASTEROIDS_SIZE_PX / 4):
            ...
            currentAsteroidsArray.push(
               createNewAsteroid(asteroidX, asteroidY, oldRadius / 3, 'small')
            );
            currentAsteroidsArray.push(
               createNewAsteroid(asteroidX, asteroidY, oldRadius / 3, 'small')
            );
...
function updateCanvas() {
   ...
   if (!playerLossStateTime && playerShip.isAlive) {
      ...
      if (playerShip.blinkingCount > 0) {
         ...
         if (playerShip.blinkingTime == 0) {
            playerShip.blinkingTime = Math.ceil(SHIP_BLINK_TIME * FRAMERATE);
   ...
   if (!playerLossStateTime) {
      ...
      // Ship thrust state
      if (playerShip.shipThrusting && playerBlinkingOn && playerShip.isAlive) {
         ...
         ctx.lineTo(
            playerShip.position.x -
               playerShip.radius *
                  ...
                  Math.cos(playerShip.position.angle),
            playerShip.position.y +
               playerShip.radius * (5 / 3) * Math.sin(playerShip.position.angle)
         );
         // right line thrust
         ctx.lineTo(
            playerShip.position.x -
